...
class CommonReportHeaderWebkit(common_report_header):
    ...
    def _get_order_by(self, data):
        return self._get_form_param('order_by', data)
    ...
    def _get_residual_amount_from_partial_reconcile(self, partial_rec_id, debit, credit):
        residual = {
                'debit' : debit,
                'credit': credit
                } 
        if not partial_rec_id:
            return res
        account_move_line_obj = self.pool['account.move.line']
        line_ids = account_move_line_obj.search(self.cursor, self.uid, [('reconcile_partial_id', '=', partial_rec_id)])
        for line in account_move_line_obj.browse(self.cursor, self.uid, line_ids):
            # Is invoice
            if debit and line.debit:
                continue
            if credit and line.credit:
                continue
            # Residual
            if debit:
                debit -= line.credit
...
SELECT l.id AS id,
            ...
            l.date_maturity,
            ptt.name AS payment_type,
            uir.invoice_id as unsolved_invoice_id,
            uinv.number as unsolved_invoice_number,
            uinv.date_invoice as unsolved_invoice_date,
            partialrec.id as partialrec_id
FROM account_move_line l
    ...
    LEFT JOIN invoice_unsolved_line_rel uir on (l.id = uir.line_id)
    LEFT JOIN account_invoice uinv on (uinv.id = uir.invoice_id)
    WHERE l.id in %s"""
        monster += (" ORDER BY %s" % (order,))
        try:
            self.cursor.execute(monster, (tuple(move_line_ids),))
            res = self.cursor.dictfetchall()
            # Residual amount from partial reconcile
            if res[0]['partialrec_id']:
                residual = self._get_residual_amount_from_partial_reconcile(res[0]['partialrec_id'], res[0]['debit'], res[0]['credit'])
                res[0]['debit'] = residual['debit']
                res[0]['credit'] = residual['credit']
                res[0]['balance'] = residual['debit'] - residual['credit']
        except Exception, exc:
            self.cursor.rollback()
            raise
        return res or []
...
####################Date specific helper ################################
    def _order_date(self, *args):
        ...
        existing_date_ids.sort(cmp=None, key=None, reverse=False)
        
        for date in existing_date_ids:
            date_work = datetime.strptime(date, "%Y-%m-%d")
            res.append((date_work.strftime("%d-%m-%Y"), date, date, None))
        
        # move lines without partners, set None for empty partner
        if not all(existing_date_ids):
