...
class CommonPartnersReportHeaderWebkit(CommonReportHeaderWebkit):
    ...
    def get_partners_move_lines_ids(self, account_id, main_filter, start, stop, target_move, order_by,
        ...
        if main_filter in ('filter_period', 'filter_no'):
            ...
            filter_from = 'date'
        elif main_filter == 'filter_date_maturity':
            filter_from = 'maturity_date'
        if filter_from:
    ...
    def _get_query_params_from_maturity_dates(self, date_start, date_stop, **args):
        """
        Build the part of the sql where clause based on the dates to print.
        ...
        sql_conditions = "  AND account_move_line.period_id not in %(period_ids)s" \
                         "  AND account_move_line.date_maturity between date(%(date_start)s) and date((%(date_stop)s))"

        return sql_conditions, search_params

    def _get_partners_move_line_ids(self, filter_from, account_id, start, stop,
                                    target_move, order_by, opening_mode='exclude_opening', 
        ...
        sql_select = "SELECT account_move_line.id, account_move_line.partner_id, account_move_line.date_maturity FROM account_move_line"
        sql_joins = ''
        ...
        if res:
            for row in res:
                if order_by == 'date_maturity':
                    final_res[row['date_maturity']].append(row['id'])
                else:
