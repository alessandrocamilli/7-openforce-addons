...
class PartnersDueRegisterWebkit(report_sxw.rml_parse, CommonPartnersReportHeaderWebkit):

    def __init__(self, cursor, uid, name, context):
        super(PartnersDueRegisterWebkit, self).__init__(cursor, uid, name, context=context)
        self.pool = pooler.get_pool(self.cr.dbname)
        self.cursor = self.cr
        
        company = self.pool.get('res.users').browse(self.cr, uid, uid, context=context).company_id
        header_report_name = ' - '.join((_('PARTNER DUE REGISTER'), company.name, company.currency_id.name))

        footer_date_time = self.formatLang(str(datetime.today()), date_time=True)
        self.localcontext.update({
            'cr': cursor,
            'uid': uid,
            'report_name': _('Partner Due Register'),
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        start_date_maturity = self._get_form_param('date_maturity_from', data)
        stop_date_maturity = self._get_form_param('date_maturity_to', data)
        ...
        order_by = self._get_form_param('order_by', data)
        exclude_reconcile = self._get_form_param('exclude_reconcile', data)
        ...
        if main_filter == 'filter_date':
            ...
            start = start_date_maturity
            stop = stop_date_maturity
        else:
            start = start_period
            stop = stop_period
        ...
        for account in self.pool.get('account.account').browse(self.cursor, self.uid, accounts):
            ...
            elif order_by == 'date_maturity':
                account.partners_order = self._order_date(ledg_lines_pids, init_bal_lines_pids)
            objects.append(account)
        
        if start_date_maturity:
            start_date = start_date_maturity
        if stop_date_maturity:
            stop_date = stop_date_maturity
        ...
        return super(PartnersDueRegisterWebkit, self).set_context(objects, data, new_ids,
                                                            report_type=report_type)

    def _compute_partner_ledger_lines(self, accounts_ids, main_filter, target_move, start, stop, order_by, exclude_reconcile, partner_filter=False):
        ...
        for acc_id in accounts_ids:
            ...
            if not move_line_ids:
                continue
            for key_id in move_line_ids:
                key_line_ids = move_line_ids.get(key_id, [])
                lines = self._get_move_line_datas(list(set(key_line_ids)))
                res[acc_id][key_id] = lines
        return res


HeaderFooterTextWebKitParser('report.account_report_partners_due_register',
                             'account.account',
                             'addons/openforce_financial_report/report/templates/account_report_partners_due_register.mako',
                             parser=PartnersDueRegisterWebkit)
