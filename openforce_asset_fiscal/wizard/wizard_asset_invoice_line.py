...
class wizard_asset_invoice_line(osv.osv_memory):
    ...
    _columns={
        #'invoice_lines': fields.many2one('account.invoice.line', 'Invoice Lines', required=True),
        'invoice_lines': fields.many2many('account.invoice.line', 'account_invoice_line_asset_move_rel', 'merge_id', 'invoice_line_id', 'Invoice Lines'),
    }
   
    def default_get(self, cr, uid, fields, context=None):
        
        res = super(wizard_asset_invoice_line, self).default_get(cr, uid, fields, context=context)
        invoice_lines = self.pool.get('account.invoice.line').search(cr, uid, [('invoice_id','=',context['active_id'])])
        #import pdb
        #pdb.set_trace()
